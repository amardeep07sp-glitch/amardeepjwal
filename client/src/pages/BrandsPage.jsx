...
import { useBrandList } from '@/features/brands/brandsApi';
...
import { brandPath } from '@/config/navConfig';
...
export default function BrandsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useBrandList({ page, limit: PAGE_SIZE });

  useSeo({ title: 'Shop by Brand', description: 'Explore our jewellery collections by brand.' });
  ...
  return (
    <PageContainer top="md" bottom="md">
      ...
      <h1 className="mb-6 flex items-center gap-2.5 text-h3 font-display font-bold text-heading sm:text-h2">
        <Award className="size-6 text-primary sm:size-7" /> Shop by Brand
      </h1>
      ...
      {isError ? (
        ...
        <EmptyState title="No brands yet" description="Brands added from the admin panel will show up here." />
      ) : (
        <>
            ...
            {data.items.map((brand) => (
              ...
              >
                <span className="flex size-16 items-center justify-center overflow-hidden rounded-full bg-primary/10 text-primary ring-1 ring-primary/20 transition-transform group-hover:scale-105">
                  {brand.logoMedia ? (
                    <img src={brand.logoMedia.secureUrl} alt={brand.name} className="size-full object-cover" />
                  ) : (
