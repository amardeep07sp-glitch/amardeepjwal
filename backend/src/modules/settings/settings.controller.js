...
import { serializeSettings, serializePublicSettings } from './settings.serializer.js';
...
export const getPublicSettings = asyncHandler(async (req, res) => {
  const settings = await settingsService.getSettings();
  res.status(200).json(new ApiResponse(200, serializePublicSettings(settings), 'Settings fetched successfully'));
});
