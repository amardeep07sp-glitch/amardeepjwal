...
import { useAddressesForSupplier, useCreateSupplierAddress, useUpdateSupplierAddress, useDeleteSupplierAddress } from './supplierAddressesApi';
import { SUPPLIER_ADDRESS_TYPE_LABELS } from './supplierSchema';

const EMPTY_FORM = { type: 'shipping', label: '', line1: '', line2: '', city: '', state: '', postalCode: '', country: 'India', phone: '' };

export function SupplierAddressBookTab({ supplierId }) {
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState(EMPTY_FORM);

  const { data: addresses } = useAddressesForSupplier(supplierId);
  ...
  const handleCreate = async () => {
    ...
    try {
      await createAddress.mutateAsync({ supplier: supplierId, ...form });
  ...
  const handleSetDefault = async (address) => {
    try {
      await updateAddress.mutateAsync({ id: address.id, isDefault: true });
    ...
    <div className="flex flex-col gap-4">
          ...
          {addresses.map((a) => (
            <li key={a.id} className="flex flex-col gap-2 rounded-lg border border-border p-3 text-sm">
              <div className="flex items-center justify-between">
                <Badge variant="outline" className="capitalize">{SUPPLIER_ADDRESS_TYPE_LABELS[a.type] ?? a.type}</Badge>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {!a.isDefault && (
                  <Button variant="outline" size="sm" onClick={() => handleSetDefault(a)}>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <FormField label="Label" description="e.g. Factory, Head Office">
