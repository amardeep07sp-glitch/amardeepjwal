...
import { useSupplierNotes, useCreateSupplierNote, useDeleteSupplierNote } from './supplierNotesApi';

export function SupplierNotesTab({ supplierId }) {
  const [content, setContent] = useState('');
  const [isPrivate, setIsPrivate] = useState(false);

  const { data: notes } = useSupplierNotes(supplierId);
  ...
  const handleAdd = async () => {
    ...
    try {
      await createNote.mutateAsync({ supplierId, content: content.trim(), isPrivate });
      setContent('');
      setIsPrivate(false);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!notes || notes.length === 0 ? (
        <EmptyState title="No notes yet" description="Internal notes about this supplier will appear here." />
      ) : (
